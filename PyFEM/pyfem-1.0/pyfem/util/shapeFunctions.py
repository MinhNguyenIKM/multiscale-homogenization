...
#
#------------------------------------------------------------------------------
# minh.nguyen@ikm.uni-hannover.de modified this code. Date: 16/11/2018
# 1. Change build-in array python to numpy array.
# 2. Add factor 1/2 to the weight.
#------------------------------------------------------------------------------
def tria_scheme( order ):

    if order is 1:
        xi     = array([[1.0/3.0,1.0/3.0]])
        weight = array([ 1.0 ])
    elif order is 3:
        r1 = 1.0/6.0
        r2 = 2.0/3.0

        xi = array([[r1,r1],[r2,r1],[r1,r2]])

        w1 = 1.0/3.0

        weight = array([w1,w1,w1])
    elif order is 7:
        r1 = 0.1012865073235
        r2 = 0.7974269853531
        r4 = 0.4701420641051
        ...
        xi = array([[r1,r1],[r2,r1],[r1,r2],[r4,r6],[r4,r4],[r6,r4],[r7,r7]])

        w1 = 0.1259391805448
        w4 = 0.1323941527885
        w7 = 0.225

        weight = array([ w1,w1,w1,w4,w4,w4,w7 ])

    return xi, 0.5 * weight  # minh.nguyen@ikm.uni-hannover.de: Please, pay attention to that factor 1/2. It is only for triangular elements.
...
# minh.nguyen@ikm.uni-hannover.de has backed up this function and changed its name to getElemShapeDataOriginal
# The problem happens for 1D bar because the jac.shape[0] and jac.shape[1] do not exist
def getElemShapeDataOriginal( elemCoords , order = 0 , method = 'Gauss' , elemType = 'Default' ):
...
# minh.nguyen@ikm.uni-hannover.de has copied the original function and modified this.
# Hence, it probably works with 1D example.
        ...
        try:
            if len(jac.shape) == 2:
                if jac.shape[0] is jac.shape[1]:
                    sData.dhdx = dot(sData.dhdxi, inv(jac))
                    sData.weight = calcWeight(jac) * weight
            elif len(jac.shape) == 1:
                sData.weight = abs(jac[0]) * weight
        except IndexError:
            print("Index Error for 1D or 2D implementation")
        elemData.sData.append(sData)
    return elemData
