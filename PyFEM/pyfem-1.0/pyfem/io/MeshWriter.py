...
class MeshWriter ( BaseModule ):
  ...
  def run( self , props , globdat ):
    ...
    # minh.nguyen@ikm.uni-hannover.de added comments here: This will calculate the average stress at nodes
    # by divided by the number of connectivity
    stress = globdat.getData("stresses", range(len(globdat.nodes)))
    # stress_cells = getattr(globdat, 'stress_elements')
    vtkfile.write('<DataArray type="Float64" Name="Nodal_sigma_xx" NumberOfComponents="1" format="ascii" >\n')
    for i in range(len(globdat.nodes)):
      vtkfile.write( str(stress[i][0]) + " \n" )
    vtkfile.write('</DataArray>\n')

    vtkfile.write('<DataArray type="Float64" Name="Nodal_sigma_yy" NumberOfComponents="1" format="ascii" >\n')
    for i in range(len(globdat.nodes)):
      vtkfile.write(str(stress[i][1]) + " \n")
    vtkfile.write('</DataArray>\n')

    vtkfile.write('<DataArray type="Float64" Name="Nodal_sigma_xy" NumberOfComponents="1" format="ascii" >\n')
    for i in range(len(globdat.nodes)):
    ...
    # vtkfile.write('<DataArray type="Float64" Name="Cell_S_xx" NumberOfComponents="1" format="ascii">\n')
    # for e in range(len(stress_cells)):
    #   vtkfile.write(str(stress_cells[e][0]) + " \n")
    # vtkfile.write('</DataArray>\n')
    #
    # vtkfile.write('<DataArray type="Float64" Name="Cell_S_yy" NumberOfComponents="1" format="ascii">\n')
    # for e in range(len(stress_cells)):
    #   vtkfile.write(str(stress_cells[e][1]) + " \n")
    # vtkfile.write('</DataArray>\n')
    #
    # vtkfile.write('<DataArray type="Float64" Name="Cell_S_xy" NumberOfComponents="1" format="ascii">\n')
    # for e in range(len(stress_cells)):
    #   vtkfile.write(str(stress_cells[e][2]) + " \n")
    # vtkfile.write('</DataArray>\n')
