...
def create_2Ddata_largedeformation_elasticity_laminate(fileinput):
	...
	for i in range(number_data):
		...
		if energy_macro == -999999:
			print("With this F the cell is penetrable - with deFmacro > 0 !!!")
			f.write("%d;\t%0.12f;\t%0.12f;\t%0.12f;\t%0.12f;\t%s\n" % (i, Fmacro[0, 0], Fmacro[0, 1], Fmacro[1, 0], Fmacro[1, 1], 'Penetrable'))
			continue
		else:
...
def create_2Ddata_largedeformation_K4_circular_inclusion(fileinput):
	...
	F12_min, F12_max = (-0.5, 0.5)
	F21_min, F21_max = (-0.5, 0.5)
	...
	data_uniform_F22 = np.random.uniform(F22_min, F22_max, number_data)
	# plot distribution
	ax = sns.distplot(data_uniform_F12,
					  bins=100,
					  kde=False,
					  color='skyblue',
					  hist_kws={"linewidth": 15, 'alpha': 1})
	ax.set(xlabel='Macro deformation gradient F12', ylabel='F12 data distribution')
	try:
		filename = fileinput
		f = open(filename, 'w+')
	except IOError as e:
...
if __name__ == '__main__':
    # create_2Ddata_largedeformation_elasticity_laminate('./dataFiles/training_data_2D_neohookean_hyperelasticity.dat')
	create_2Ddata_largedeformation_K4_circular_inclusion('./dataFiles/training_data_2D_neohookean_K4_circular_inclusion2_19102018.dat')
