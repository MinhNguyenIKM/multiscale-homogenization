...
nntobsf('tramnmx','Use MAPMINMAX instead.')
...
if nargin ~= 3
  error('NNET:tramnmx:Arguments','Wrong number of arguments.');
end

equal = minp==maxp;
nequal = ~equal;
if sum(equal) ~= 0
  warning('NNET:tramnmx:Arguments','Some maximums and minimums are equal. Those inputs won''t be transformed.');
