...
nntobsf('prestd','Use MAPSTD instead.')
...
if nargin > 2
  error('NNET:prestd:Arguments','Wrong number of arguments.');
end
...
nequal = ~equal;
if sum(equal) ~= 0
  warning('NNET:prestd:Arguments','Some standard deviations are zero. Those inputs won''t be transformed.');
...
if nargin==2
  ...
  nequal = ~equal;
  if sum(equal) ~= 0
    warning('NNET:prestd:Arguments','Some standard deviations are zero. Those targets won''t be transformed.');
