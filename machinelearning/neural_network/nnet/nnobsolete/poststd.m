...
nequal = ~equal;
if sum(equal) ~= 0
  warning('NNET:poststd:Arguments','Some standard deviations are zero. Those inputs won''t be transformed.');
...
if nargin==6
  ...
  nequal = ~equal;
  if sum(equal) ~= 0
    warning('NNET:poststd:Arguments','Some standard deviations are zero. Those targets won''t be transformed.');
