...
nequal = ~equal;
if sum(equal) ~= 0
  warning('NNET:postmnmx:Arguments','Some maximums and minimums are equal. Those inputs won''t be transformed.');
...
if nargin==6
  ...
  nequal = ~equal;
  if sum(equal) ~= 0
    warning('NNET:postmnmx:Arguments','Some maximums and minimums are equal. Those targets won''t be transformed.');
