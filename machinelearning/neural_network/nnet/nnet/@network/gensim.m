...
% $Revision: 1.11.4.5 $ $Date: 2007/11/09 20:49:12 $
...
% Check ST
if (st <= 0) && (st ~= -1),
  error('NNET:Arguments','Sample time must be -1, or a positive number.')
end
if (st == -1) && (net.numInputDelays | net.numLayerDelays)
  error('NNET:Arguments','Sample time cannot be -1 because the network contains delays.')
end
st = num2str(st);
...
for i=1:net.numInputs, inputNames{i} = sprintf('x{%g}',i); end
...
%======================================================================
function genNetworkInput(net,inputIndex,pos,inputName,processInputName,processedInputName, ...
  netNameL,sampleTime)

  y = pos*40;
  add_block( ...
    'built-in/Inport',[netNameL '/' inputName],...
    'port',sprintf('%g',inputIndex), ...
...
%======================================================================
function genNetworkOutput(net,outputIndex,pos,processOutputName,outputName,netNameL,sampleTime)
