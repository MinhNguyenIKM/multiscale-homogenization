function net=network(numInputs,numLayers,biasConnect,inputConnect, ...
  layerConnect,outputConnect,ignore)
...
% $Revision: 1.9.4.8 $ $Date: 2007/11/09 20:49:14 $

% SPECIAL CASE - Remaking a network from a struct
if (nargin == 1)
  if isa(numInputs,'struct')
    net = class(numInputs,'network');
    return;
  elseif isa(numInputs,'network')
    net = numInputs;
    return;
  end
end
...
if ~isposint(numInputs)
  error('NNET:Arguments','NumInputs must be 0 or a positive integer.')
end
if nargin < 2
  numLayers = 0;
end
if ~isposint(numLayers)
  error('NNET:Arguments','NumLayers must be 0 or a positive integer.')
end
if nargin < 3
  biasConnect = zeros(numLayers,1);
end
if ~isbool(biasConnect,numLayers,1)
  error('NNET:Arguments','BiasConnect must be a NumLayers-by-1 boolean matrix.')
end
if nargin < 4
  inputConnect = zeros(numLayers,numInputs);
end
if ((numLayers == 0) || (numInputs == 0)) && isempty(inputConnect)
  inputConnect = zeros(numLayers,numInputs);
end
if ~isbool(inputConnect,numLayers,numInputs)
  error('NNET:Arguments','InputConnect must be a NumLayers-by-NumInputs boolean matrix.')
end
if nargin < 5
  layerConnect = zeros(numLayers,numLayers);
end
if (numLayers == 0) && isempty(layerConnect)
  layerConnect = zeros(0,0);
end
if ~isbool(layerConnect,numLayers,numLayers)
  error('NNET:Arguments','LayerConnect must be a NumLayers-by-NumLayers boolean matrix.')
end
if nargin < 6
  outputConnect = zeros(1,numLayers);
end
if ~isbool(outputConnect,1,numLayers)
  error('NNET:Arguments','OutputConnect must be a 1-by-NumLayers boolean matrix.')
end
if nargin == 8
  nntwarn('"targetConnect" is obsolete. Only "outputConnect" is required."');
end
