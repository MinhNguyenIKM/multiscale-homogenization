function [Y,Xf,Af,E,perf]=sim(net,X,Xi,Ai,T)
%SIM Simulate a neural network.
%
%  Syntax
%
%    [Y,Xf,Af,E,perf] = sim(net,X,Xi,Ai,T)
%    [Y,Xf,Af,E,perf] = sim(net,{Q TS},Xi,Ai,T)
%    [Y,Xf,Af,E,perf] = sim(net,Q,Xi,Ai,T)
%
%  Description
%
%    SIM simulates neural networks.
%
%    [Y,Xf,Af,E,perf] = SIM(net,X,Xi,Ai,T) takes,
%      NET  - Network.
%      P    - Network inputs.
...
%
%    Here NEWLIN is used to create a linear layer with a 3-element
%    input, 2 neurons.
%
...
% $Revision: 1.8.4.7 $ $Date: 2008/01/10 21:11:17 $
...
if ~isempty(net.hint.noWeights), error('NNET:Arguments','Layer %g has no weights.',net.hint.noWeights(1)), end

switch nargin
  case 2, [err,X,Xi,Ai,T,Q,TS,matrixForm] = simargs(net,X);
  case 3, [err,X,Xi,Ai,T,Q,TS,matrixForm] = simargs(net,X,Xi);
  case 4, [err,X,Xi,Ai,T,Q,TS,matrixForm] = simargs(net,X,Xi,Ai);
  case 5, [err,X,Xi,Ai,T,Q,TS,matrixForm] = simargs(net,X,Xi,Ai,T);
end
...
% Performance
if nargout >= 5
  perf = feval(net.performFcn,E,Y,net,net.performParam);
end
