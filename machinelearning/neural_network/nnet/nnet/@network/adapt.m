...
%  $Revision: 1.13.4.5 $ $Date: 2007/11/09 20:49:10 $
...
if ~isa(net,'network')
  error('NNET:Arguments','First argument is not a network.');
end
if net.hint.zeroDelay
  error('NNET:Arguments','Network contains a zero-delay loop.')
end
switch nargin
  case 2,[err,P,T,Pi,Ai,Q,TS,matrixForm] = trainargs(net,P,[],[],[]);
  case 3,[err,P,T,Pi,Ai,Q,TS,matrixForm] = trainargs(net,P,T,[],[]);
  case 4,[err,P,T,Pi,Ai,Q,TS,matrixForm] = trainargs(net,P,T,Pi,[]);
  case 5,[err,P,T,Pi,Ai,Q,TS,matrixForm] = trainargs(net,P,T,Pi,Ai);
end
...
if isempty(adaptFcn)
  error('NNET:Property','Network "adaptFcn" is undefined.')
end

% Combined, processed, delayed inputs
Pc = [Pi P];
Pp = processinputs(net,Pc);
Pd = calcpd(net,TS,Q,Pp);

% Layer targets
Tl = expandrows(T,net.hint.outputInd,net.numLayers);
