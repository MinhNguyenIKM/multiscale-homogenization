...
if ~isa(b,'cell'),
 error('NNET:Arguments','Input argument must be a cell array');
end
if size(b,2) ~= 1
  error('NNET:Arguments','Input argument must have only one column.')
end
n = size(b,1);
s = cell(n,ts);
for i=1:n
  ...
  q2 = q1/ts;
  if (q2*ts ~= q1)
    error('NNET:Arguments','Matrix cannot be divided into TS parts equally.');
  end
  for j=1:ts
