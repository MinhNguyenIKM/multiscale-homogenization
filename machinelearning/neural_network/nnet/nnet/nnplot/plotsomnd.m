function result = plotsomnd(varargin)
...
%  Description
%
%    PLOTSOMND(NET) plots a SOM layer showing neurons as gray-blue patches
%    and their direct neighbor relations with red lines. The neighbor
%    patches are colored from black to yellow to show how close each
%    neuron's weight vector is to its neighbors.
%
%  Example
%
%    load iris_dataset
...
%    plotsomnd(net);
%
% See also plotsomplanes, plotsomhits, plotsomn
...
if (nargin == 1) && strcmp(varargin{1},'info')
  info.name = mfilename;
  info.title = 'SOM Neighbor Distances';
  info.type = 'Plot';
  info.version = 6;
  result = info;
  return;
...
set(fig,'name','SOM Neighbor Distances (plotsomnd)');
