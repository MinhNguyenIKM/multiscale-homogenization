...
% $Revision: 1.1.6.6 $ $Date: 2007/11/09 20:50:53 $
...
if (~isa(p,'cell'))
  error('NNET:Arguments','Inputs is not a matrix or cell array of matrices.');
end
TS = size(p,2);
Ni = size(p,1);
Q = size(p{1,1},2);
...
for i=1:Ni
  for j=1:TS
    if size(p{i,j},1) ~= r(i)
      error('NNET:Arguments','Input matrices in the same row have a different numbers of rows.')
    end
    if size(p{i,j},2) ~= Q
      error('NNET:Arguments','All Input matrices do not have the same number of columns.')
    end
  end
end
...
if (~isa(t,'cell'))
  error('NNET:Arguments','Targets is not a matrix or cell array of matrices.');
end
Nl = size(t,1);
for i=1:Nl
  s(i) = size(t{i,1},1);
...
S = sum(s);
if size(t,2) ~= TS
  error('NNET:Arguments','Inputs and Targets have different numbers of time steps (columns of matrices).')
end
for i=1:Ni
  for j=1:TS
    if size(t{i,j},1) ~= s(i)
      error('NNET:Arguments','Target matrices in the same row have different numbers of rows.')
    end
    if size(t{i,j},2) ~= Q
      error('NNET:Arguments','Target matrices do not have the same number of columns as Input matrices.')
    end
  end
end
...
if (~isa(pi,'cell'))
  error('NNET:Arguments','Initial Inputs is not a matrix or cell array of matrices.');
end
id = size(pi,2);
if size(pi,1) ~= Nl
  error('NNET:Arguments','Initial Inputs must have as many rows of matrices as Inputs.');
end
for i=1:Ni
  for j=1:id
    if size(pi{i,j},1) ~= r(i)
      error('NNET:Arguments','Initial Input matrices have different numbers of rows from Input matrices in the same row.')
    end
    if size(pi{i,j},2) ~= Q
      error('NNET:Arguments','Initial Input matrices do not have the same number of columns as Input matrices.')
    end
  end
end
