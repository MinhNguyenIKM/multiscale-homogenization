...
if length(b) ~= numLayers
  error('NNET:Arguments','Must provide same number of biases as weights.')
end
if (nargin > 3) && (length(tf) ~= length(b))
  error('NNET:Arguments','Must provide same number of transfer functions as weights and biases.')
end
