...
if size(pr,1) ~= size(w1,2)-size(w1,1), error('NNET:Arguments','PR and W1 sizes do not match.'), end
if size(w1,1) ~= size(b1,1), error('NNET:Arguments','W1 and B1 sizes do not match.'), end
if size(b1,2) ~= 1, error('NNET:Arguments','B1 must have one column.'), end
if size(w1,1) ~= size(w2,2), error('NNET:Arguments','W1 and W2 sizes do not match.'), end
if size(w2,1) ~= size(b2,1), error('NNET:Arguments','W2 and B2 sizes do not match.'), end
if size(b2,2) ~= 1, error('NNET:Arguments','B2 must have one column.'), end
