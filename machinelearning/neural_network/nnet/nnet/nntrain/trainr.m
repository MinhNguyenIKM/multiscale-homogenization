function [net,tr] = trainr(net,tr,trainV,valV,testV,varargin)
%TRAINR Random order weight/bias training.
%
%  Syntax
%  
%    [net,tr,Ac,El] = trainr(net,tr,trainV,valV,testV)
%    info = trainr('info')
%
%  Description
%
...
% $Revision: 1.1.6.8 $  $Date: 2008/03/13 17:33:35 $

%% Info
if strcmp(net,'info')
  info.function = mfilename;
  info.title = 'Random Order Weight/Bias Learning Rules';
  info.type = 'Training';
  info.version = 6;
...
for epoch=0:epochs
  ...
  for qq=1:Q
    ...
    % Gradient
    if (needGradient)
      gE = cell(net.numOutputs,TS);
      gE(net.hint.outputInd,:) = feval(performFcn,'dy',E,Y,X,perf,net.performParam);
