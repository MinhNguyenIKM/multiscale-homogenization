function [net,tr,Ac,El]=trains(net,PD,Tl,Ai,Q,TS,varargin)
...
% $Revision: 1.1.6.7 $  $Date: 2008/03/13 17:33:37 $
...
if ischar(net)
  switch (net)
    case 'name'
      net = 'Sequential Order Weight-Bias Learning Rules.';
    case 'pnames',
    net = {'passes'};
    case 'pdefaults',
    trainParam.passes = 1;
...
if (~isa(passes,'double')) || (~isreal(passes)) || (any(size(passes)) ~= 1) || ...
  ...
  error('NNET:Arguments','Passes is not a positive integer.')
end
...
Yl = cell(numLayers,TS);
El = cell(numLayers,TS);
...
for pass=1:passes
  for ts=1:TS
    ...
    Yl(:,ts) = processoutputs(net,Al(:,ts));
    El(:,ts) = calce1(net,Yl(:,ts),Tl(:,ts));
    tr.perf(ts) = feval(performFcn,El(:,ts),Yl(:,ts),getx(net),performParam);
  
    % Gradient
    if (needGradient)
      gE(net.outputConnect,:) = feval(net.performFcn,'dy',El(net.outputConnect,ts), ...
        Ac(net.outputConnect,ts+AcInd),getx(net),tr.perf(ts),net.performParam);
      [gB,gIW,gLW] = calcgrad(net,Q,PD(:,:,ts),BZ,IWZ,LWZ,N,Ac(:,ts+AcInd),gE,1);
    end
